...
function parse_query(query) {
  try {
    ...
    for (const section of query.split("&")) {
      const [key2, value] = section.split("=");
      if (!key2)
        continue;
...
function insert(dict2, key2, value) {
  return map_insert(key2, value, dict2);
}
...
function key_set(list3, key2, value) {
  if (list3.hasLength(0)) {
    return toList([[key2, value]]);
  } else if (list3.atLeastLength(1) && isEqual(list3.head[0], key2)) {
    let k = list3.head[0];
    let rest$1 = list3.tail;
    return prepend([key2, value], rest$1);
  } else {
    let first$1 = list3.head;
    let rest$1 = list3.tail;
    return prepend(first$1, key_set(rest$1, key2, value));
  }
}
...
function percent_encode2(value) {
...
function set_header(request, key2, value) {
  let headers = key_set(request.headers, lowercase2(key2), value);
...
var Song = class extends CustomType {
  constructor(title, artists, album, source) {
    super();
    this.title = title;
...
function song_decoder(v) {
  ...
  return decode4(
    (var0, var1, var2, var3) => {
      return new Song(var0, var1, var2, var3);
    },
    field("title", string2),
...
function layout(children2) {
  return div(
    toList([
      class$("bg-zinc-800 min-w-screen min-h-screen text-pink-100")
    ]),
    toList([
      div(
        toList([
          ...
          let _pipe$3 = replace_error(
            ...
            toList([
              new DecodeError(
                ...
                "A list with" + to_string2(length3(list3)) + "elements",
...
function json_body(encoder, decoder) {
  return new RouteBody(
    ...
    new BodyConverter(
      ...
      (body) => {
        let _pipe = decode2(body, decoder);
        return map_error(
          _pipe,
          (err) => {
...
function encode(body, value) {
  let _pipe = value;
  return body.converter.encoder(_pipe);
}
function decode7(body, value) {
  let _pipe = value;
  return body.converter.decoder(_pipe);
}
function get_type(body) {
  return body.btype;
}
      ...
      (() => {
        ...
        let _pipe$1 = set_body(
          ...
          (() => {
            ...
            let _pipe$2 = encode(_pipe$12, body);
      ...
      (() => {
        ...
        return set_body(
          ...
          (() => {
            ...
            let _pipe$2 = encode(_pipe$1, body);
            return to_string3(_pipe$2);
          })()
        );
      })()
                  ...
                  (body) => {
                    ...
                    let _pipe$2 = then$(
                      ...
                      (value) => {
                        ...
                        let _pipe$3 = decode7(_pipe$22, value);
...
function search_decoder(value) {
  return try$(
    key_find(value, "q"),
    (search4) => {
      return try$(
        key_find(value, "token"),
        (token) => {
...
var Model2 = class extends CustomType {
  constructor(token, last_search, results) {
    super();
    this.token = token;
...
function update(model, msg) {
  if (msg instanceof SearchSongs) {
    let q = msg.search;
    return [
      model.withFields({ last_search: q }),
